...
#                                   % (mac, ip))
    ...
    def _convert_sgr_to_iptables_rules(self, security_group_rules):
        ...
        self._allow_dhcp_rules(security_group_rules, iptables_rules)
        self._drop_iptables_rules(security_group_rules, iptables_rules)
        for rule in security_group_rules:
            if self.enable_ipset:
            ...
            if rule.get('action') == 'allow' and not rule.get('source_port_range_min'):
    ...
    def _drop_iptables_rules(self, security_group_rules, iptables_rules):
        for rule in security_group_rules:
            # add drop rules in the top.
    ...
    def _allow_dhcp_rules(self, security_group_rules, iptables_rules):
        for rule in security_group_rules:
            # add drop rules in the top.
            if (rule.get('source_ip_prefix') and
                rule.get('source_port_range_min') and
