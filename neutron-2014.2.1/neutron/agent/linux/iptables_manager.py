...
class IptablesManager(object):
    ...
    def __init__(self, _execute=None, state_less=False,
        ...
        if not state_less:
            ...
            # Add a neutron-l3-agent-internal chain and neutron-l3-agent-PREROUTING jump to the chain.
            self.ipv4['raw'].add_chain('internal')
            self.ipv4['raw'].add_rule('PREROUTING', '-j $internal', top = True)

            # Add a neutron-l3-agent-limit-bw chain and neutron-l3-agent-PREROUTING jump to the chain.
            self.ipv4['raw'].add_chain('limit-bw')
            self.ipv4['raw'].add_rule('PREROUTING', '-j $limit-bw')
