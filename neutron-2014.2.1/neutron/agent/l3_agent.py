...
class L3NATAgent(firewall_l3_agent.FWaaSL3AgentRpcCallback,
    ...
    OPTS = [
        ...
        cfg.StrOpt('external_network_bridge', default='br-ex',
                   help=_("Name of bridge used for external network "
                          "traffic.")),
        cfg.StrOpt('l3_ha_bridge', default='br-ha',
                   help=_("Name of bridge used for l3 ha network "
                   "traffic.")),
    ...
    def process_router(self, ri):
        ...
        if (not ri.router['distributed'] or
            ...
            self.router_allow_internal_rules(ri, internal_cidrs, action= 'add_rules')
        ...
        if ex_gw_port:
            ...
            self.router_limit_bandwith_rules(ri)
    ...
    def router_limit_bandwith_rules(self, ri):
        """Configures Router bandwith.

           Add rules limit traffic to each router .
        """
        ri.iptables_manager.defer_apply_off()
        ri.iptables_manager.ipv4['raw'].empty_chain('limit-bw')
        
        rules = self.bandwith_raw_rules(ri.router['bandwith'])        
        for rule in rules:
            ri.iptables_manager.ipv4['raw'].add_rule(*rule)
        ri.iptables_manager.apply()

    def remove_router_limit_bandwith_rules(self, ri):

        ri.iptables_manager.defer_apply_off()
        ri.iptables_manager.ipv4['raw'].empty_chain('limit-bw')
        ri.iptables_manager.apply()

    def bandwith_raw_rules(self, bw = DEFAULT_BANDWITH):
        rules = []
        limit_burst = 100
	if not bw:
	    return rules
        if bw < DEFAULT_BANDWITH:
            bw = DEFAULT_BANDWITH
	if bw > MAX_BANDWITH:
	    bw = MAX_BANDWITH
        bandwith = int((bw * 1000) // (1.5 * 8))
        rules.append(('limit-bw', '-m limit --limit %d --limit-burst %d -j ACCEPT' % (bandwith, limit_burst)))
        rules.append(('limit-bw', '-j DROP'))
        return rules

    def router_allow_internal_rules(self, ri, internal_cidrs, action):
        """Configures Router bandwith.
           Add rules allow cidr traffic to each router .
        """
        ri.iptables_manager.defer_apply_off()
        ri.iptables_manager.ipv4['raw'].empty_chain('internal')
 
        if action == 'add_rules':
            rules = self.router_internal_network_rules(ri, internal_cidrs)
            for rule in rules:
                ri.iptables_manager.ipv4['raw'].add_rule(*rule)
        ri.iptables_manager.apply()

    def router_internal_network_rules(self, ex_gw_ip, internal_cidrs):
        rules = []
        for cidr in internal_cidrs:
            rules.append(('internal', '-d %s -j ACCEPT' % cidr))
        return rules
    ...
    def external_gateway_removed(self, ri, ex_gw_port, interface_name):
        ...
        self.remove_router_limit_bandwith_rules(ri)
