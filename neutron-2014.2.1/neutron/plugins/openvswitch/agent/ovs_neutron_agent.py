...
class OVSNeutronAgent(n_rpc.RpcCallback,
    ...
    def __init__(self, integ_br, tun_br, local_ip,
                 ...
                 minimize_polling=False, l3_ha_bridge='br-ha',
        ...
        self.ha_br = ovs_lib.OVSBridge(l3_ha_bridge, self.root_helper)
        ...
        self.dvr_agent.setup_dvr_flows_on_integ_tun_br(ovs_restarted=False)

        # Collect additional bridges to monitor
        self.ancillary_brs = self.setup_ancillary_bridges(integ_br, l3_ha_bridge, tun_br)
    ...
    def port_bound(self, port, net_uuid,
        ...
        if network_type in constants.TUNNEL_NETWORK_TYPES:
            self.ha_br.set_db_attribute("Port", port.port_name, "tag",
                                         str(segmentation_id))
            if port.ofport != -1:
                self.ha_br.delete_flows(in_port=port.ofport)
        else:
            ...
            if cur_tag != str(lvm.vlan):
                ...
                if cur_tag == DEAD_VLAN_TAG:
                    return
                LOG.info(_("Port bound: %(port)s network type "
                          "%(network_type)s for net-id=%(net_uuid)s"
                          "current tag is %(cur_tag)s, new tag is %(dl_vlan)s"),
                          {'network_type': lvm.network_type,
                          'port': port.vif_id,
                          'net_uuid': net_uuid,
                          'cur_tag': cur_tag, 'dl_vlan': lvm.vlan})
    ...
    def setup_integration_br(self, ovs_restarted=False):
        ...
        if cfg.CONF.OVS.reset_ovs or ovs_restarted:
    ...
    def setup_l3_ha_br(self, ovs_restarted=False):
        '''Setup the l3 ha bridge.

        Create patch ports and remove all existing flows.

        :param bridge_name: the name of the l3 ha bridge.
        :returns: the l3 ha bridge
        '''
        # Ensure the l3 ha bridge is created.
        # ovs_lib.OVSBridge.create() will run
        #   ovs-vsctl -- --may-exist add-br BRIDGE_NAME
        # which does nothing if bridge already exists.
        self.ha_br.create()
        self.ha_br.set_secure_mode()

        if cfg.CONF.OVS.reset_ovs or ovs_restarted:
            self.ha_br.delete_port(cfg.CONF.OVS.int_peer_patch_port)
            self.ha_br.remove_all_flows()
        # switch all traffic using L2 learning
        self.ha_br.add_flow(priority=1, actions="normal")
        # Add a canary flow to int_br to track OVS restarts
        self.ha_br.add_flow(table=constants.CANARY_TABLE, priority=0,
                             actions="drop")

    def setup_ancillary_bridges(self, integ_br, l3_ha_bridge, tun_br):
        '''Setup ancillary bridges - for example br-ex.'''
        ovs_bridges = set(ovs_lib.get_bridges(self.root_helper))
        # Remove all known bridges
        ovs_bridges.remove(integ_br)
        ovs_bridges.remove(l3_ha_bridge)
        if self.enable_tunneling:
    ...
    def setup_tunnel_br(self, tun_br_name=None, ovs_restarted=False):
        ...
        if self.ha_br:
            self.patch_tun_ofport_ha = self.ha_br.add_patch_port(
                cfg.CONF.OVS.int_peer_patch_port_ha, cfg.CONF.OVS.tun_peer_patch_port_ha)
            self.patch_int_ofport_ha = self.tun_br.add_patch_port(
                cfg.CONF.OVS.tun_peer_patch_port_ha, cfg.CONF.OVS.int_peer_patch_port_ha)
        ...
        if self.patch_int_ofport_ha and int(self.patch_int_ofport_ha) > 0:
            self.tun_br.add_flow(priority=1,
                                 in_port=self.patch_int_ofport_ha,
                                 actions="resubmit(,%s)" %
                                 constants.PATCH_LV_TO_TUN)
    ...
    def scan_ports(self, registered_ports, updated_ports=None):
        ...
        cur_ports1 = self.ha_br.get_vif_port_set()
        cur_ports.update(cur_ports1)
    ...
    def test_physical_vlan(self, vif_port, network_type, physical_network,
                           segmentation_id, fixed_ips, subnet):
        ovs_bridges = ovs_lib.get_bridges(self.root_helper)
        if self.bridge_mappings.get(physical_network):
            ...
            if test_port not in br.get_port_name_list():
                br.add_port_internal(test_port)
            br.set_db_attribute("Port", test_port, "tag", str(segmentation_id))
            gateway = subnet['gateway_ip']
            net = netaddr.IPNetwork(subnet['cidr'])
            local_ip = fixed_ips[0]['ip_address']
            ip_cidr = '%s/%s' % (local_ip, net.prefixlen)
            ping_cmd = 'ping -I %s %s -c 2' %(test_port, gateway)
            try:
                utils.execute(['ip', 'link', 'set', 'dev', test_port, 'up'], self.root_helper)
                utils.execute(['ip', 'addr', 'add', ip_cidr, 'dev', test_port], self.root_helper)
                if commands.getstatusoutput(ping_cmd)[0]:
                    LOG.info("****************************** "
                             "The ip %(local_ip)s in vlan %(segmentation_id)s can "
                             "not connected the gateway %(gateway)s "
                             "******************************"
                             %{'local_ip':local_ip,
                               'gateway':gateway,
                               'segmentation_id':segmentation_id})
            except Exception as e:
                LOG.debug("Test physinal vlan error: %s" %e)
            finally:
                br.delete_port(test_port)
    ...
    def treat_devices_added_or_updated(self, devices, ovs_restarted):
        ...
        # to avoid change the flows for it.
            self.rebuild_local_vlan_mapping(devices_details_list, ovs_restarted)
        ...
        for details in devices_details_list:
            ...
            port1 = self.ha_br.get_vif_port_by_id(device)
            if not port:
                if not port1:
            ...
            if 'port_id' in details:
                ...
                if port:
                    if details['network_id'] not in self.local_vlan_map or ovs_restarted:
                        subnet_id = details['fixed_ips'][0]['subnet_id']
                        try:
                            subnet_details = self.plugin_rpc.get_subnet_details(
                                          self.context,
                                          subnet_id,
                                          cfg.CONF.host)
                        except Exception as e:
                            raise Exception("Can not get subnet %(subnet_id)s details, because of %(e)s"
                                              %{'subnet_id': subnet_id, 'e': e})
                        self.test_physical_vlan(port,
                    ...
                    self.treat_vif_port(port, details['port_id'],
                                        ...
                                        ovs_restarted)
                if port1:
                    self.treat_vif_port(port1, details['port_id'],
                                        details['network_id'],
                                        details['network_type'],
                                        details['physical_network'],
                                        details['segmentation_id'],
    ...
    def rebuild_local_vlan_mapping(self, details, ovs_restarted):
        ...
        # We not need provision_local_vlan
        # Save the local vlan mapping for the exist ports
        # Pop the exits port tags from local vlans
        if self.iter_num != 0 or ovs_restarted:
            return

        for detail in details:
            device = detail['device']
            LOG.debug("Processing rebuild_local_vlan_mapping"
                      " for port: %(device)s",
                      {'device': device})
            port = self.int_br.get_vif_port_by_id(device)
            port1 = self.ha_br.get_vif_port_by_id(device)
            if not port and not port1:
                # The port disappeared and cannot be processed
                LOG.info(_("Port %s was not found on the"
                            " integration bridge "
                           "and will therefore not be processed by"
                           " rebuild_local_vlan_mapping"),
                           {'device': device})
                continue
            network_id = detail['network_id']
            if not network_id:
                # The port network_id not correct and cannot be processed
                LOG.info(_("network_id not configured for port %s "
                           "and will therefore not be processed by"
                           " rebuild_local_vlan_mapping"),
                           {'device': device})
                continue
            cur_tag = self.int_br.db_get_val("Port", port.port_name, "tag")
            if cur_tag == '[]' or cur_tag == DEAD_VLAN_TAG:
            ...
            tag = int(cur_tag)
            self.local_vlan_map[network_id] = LocalVLANMapping(tag,
                                                detail['network_type'],
                                                detail['physical_network'],
                                                detail['segmentation_id'])
            LOG.info(_("cur_tag is %(vlan)s for port %(device)s"
                        "of %(segmentation_id)s "
                           "on the integration bridge "
                           " by rebuild_local_vlan_mapping"),
                        {'device': device, 'vlan': cur_tag,
                         'segmentation_id': detail['segmentation_id']})
            if tag in self.available_local_vlans:
                self.available_local_vlans.remove(tag)
    ...
    def rpc_loop(self, polling_manager=None):
        ...
        while self.run_daemon_loop:
            ...
            if ovs_restarted:
                ...
                self.setup_l3_ha_br(ovs_restarted)
