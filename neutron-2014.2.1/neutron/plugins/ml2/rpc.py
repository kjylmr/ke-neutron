...
class RpcCallbacks(n_rpc.RpcCallback,
    ...
    def get_subnet_details(self, rpc_context, **kwargs):
        """Agent requests subnet details."""
        subnet_id = kwargs.get('subnet_id')
        host = kwargs.get('host')
        LOG.debug("Subnet %(subnet_id)s details requested by agent with  host %(host)s",
                  {'subnet_id': subnet_id, 'host': host})
        plugin = manager.NeutronManager.get_plugin()
        return plugin.get_subnet(rpc_context, subnet_id)
